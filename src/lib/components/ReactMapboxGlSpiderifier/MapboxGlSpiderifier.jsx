...
class MapboxGlSpiderifier extends Component {
	...
	_generateSpiderParams(props = this.props) {
		const { circleSpiralSwitchover, animate, animationSpeed, showingLegs } = props;
		const count = this._getMarkersCount(props);
		if (!count) {
			return null;
		}
		...
		const markersProps =
			count >= circleSpiralSwitchover ? this._generateSpiralParams(props) : this._generateCircleParams(props);
	...
	_generateSpiralParams(props) {
		const count = this._getMarkersCount(props);
		const { spiralFootSeparation, spiralLengthFactor, spiralLengthStart } = props;
	...
	_getSpiderifierMarkers() {
		...
		return this._getNotNullChildren().map((child, index) => {
			...
			if (params) {
				return (
					<SpiderifierElement key={index} coordinates={coordinates} legStyles={legStyles} {...params} {...eventHanders}>
						{child}
					</SpiderifierElement>
				);
			}
